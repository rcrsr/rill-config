...
- **Shared dev assets move from a copied `dev/` directory to the `@rcrsr/rill-dev` devDependency.** The conformance checker, the custom oxlint rules, and `REPO-STANDARDS.md` were previously copied into this repository byte-identically by `rill/dev/apply.sh` and policed by a CI drift check. They now install as a package and upgrade by version. `check:standards` runs `rill-check-standards`, `test:rules` runs `rill-test-rules`, and `.oxlintrc.json` loads the rules through the `@rcrsr/rill-dev/lint-rules` subpath export rather than a relative path. The `dev/` directory and the `Dev assets are current` CI job are removed; `dev/bootstrap.sh` moves to `scripts/bootstrap.sh`, staying local because it performs the install that fetches the package. No change to published behavior: this repository's `files` field ships `dist` only, and none of this reaches consumers.
- **The compatibility workflow covers `@rcrsr/rill-dev` as well as `@rcrsr/rill`.** A rill-dev release breaks conformance, linting, or the rule suite rather than the build, so the nightly job now runs `test:rules`, `check:lint`, and `check:standards` on top of build and test.
- **`rill/no-duplicate-error-id` is enabled** for `src/**/*.ts`, alongside the already-enabled `rill/no-spec-id-reference`. It matches nothing today, since this package constructs only `ConfigError` subclasses and the rule binds to rill's `RuntimeError`. It is on so that stops being true loudly rather than silently, the day a module imports `RuntimeError`.
