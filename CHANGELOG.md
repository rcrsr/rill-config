...
- **`@rcrsr/rill-dev` 0.1.0 to 0.2.0:** The conformance checker now ships `baseline.json`, a published copy of rill's canonical tooling pins, so elements that compare this repository against rill's tree stop reporting unchecked from a single checkout. Tree-only coverage here goes from 52 checked to 63, with the unchecked count falling from 21 to 14; every newly decided element passes without a change to this tree. `STD-LINT-1`, `STD-LINT-5`, `STD-LINT-9`, `STD-PM-2`, `STD-DEP-1`, `STD-DEP-2`, and `STD-DEP-5` resolve against the baseline; `STD-PM-6`, `STD-PROC-7`, and `STD-SUP-2` are now decided from the tree; `STD-LINT-3` gains a JSONC reader that checks the override's `files` glob actually covers `src`, rather than the rule's severity alone. `STD-DEP-3` and `STD-DEP-4` split out of the former blanket `STD-DEP-1..5` skip and report N/A with their stated conditions. The summary line now carries the checker's own version, so a count difference between two repositories reads as version skew rather than a conformance gap. No change to `src/`, to the test suite, or to published behavior. ([#21](https://github.com/rcrsr/rill-config/pull/21))
